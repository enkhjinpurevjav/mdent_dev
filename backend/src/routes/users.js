...
/**
 * POST /api/users/:id/reception-schedule/bulk
 * Body:
 * {
 *   branchId: number,
 *   dateFrom: "YYYY-MM-DD",
 ...
 */
router.post("/:id/reception-schedule/bulk", async (req, res) => {
  const receptionId = Number(req.params.id);
  if (!receptionId || Number.isNaN(receptionId)) {
    return res.status(400).json({ error: "Invalid receptionist id" });
  }
  ...
  try {
    ...
    for (const [dateStr, shiftType] of Object.entries(shiftTypeByDate)) {
      ...
      if (existing) {
        await prisma.receptionSchedule.update({
          where: { id: existing.id },
          data: { startTime: times.startTime, endTime: times.endTime, note: null },
        });
        updated += 1;
      } else {
        await prisma.receptionSchedule.create({
          data: {
            receptionId,
            branchId: bid,
            date: day,
    ...
    console.error("POST /api/users/:id/reception-schedule/bulk error:", err);
    return res.status(500).json({ error: "Failed to bulk-save reception schedule" });
  }
});
