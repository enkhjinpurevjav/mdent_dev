...
// ─── Doctor dashboard (performance) endpoint ─────────────────────────────────

/** Normalize gender value to "male" | "female" | "unknown". */
...
/** Compute age in years at a reference date. Returns null if birthDate is absent. */
function ageAt(birthDate, refDate) {
  ...
  if (mDiff < 0 || (mDiff === 0 && refDate.getUTCDate() < birthDate.getUTCDate())) age--;
  return age;
}

/** Minimum BARTER payment amount before the excess contributes to doctor sales. */
const DASHBOARD_BARTER_THRESHOLD_MNT = 800_000;

/**
 * Compute sales + income contribution from a single invoice for a given bucket range.
...
function calcDashboardInvoiceContribution(inv, bucketStart, bucketEnd, homeBleachingDeductAmountMnt, cfg) {
  ...
  if (hasOverride) {
    ...
    const barterExcess = Math.max(0, barterSum - DASHBOARD_BARTER_THRESHOLD_MNT);
    if (barterExcess > 0) {
      const allocatedBarterExcess = barterExcess * nonImagingRatio;
  ...
  for (const it of serviceItems) {
    ...
    if (service?.category === "IMAGING") {
      if (it.meta?.assignedTo === "DOCTOR") income += lineNet * (imagingPct / 100);
      continue;
    }
 ...
 *
 * Returns bucketed performance metrics for the authenticated doctor.
 * Query params: startDate=YYYY-MM-DD, endDate=YYYY-MM-DD, bucket=month|week
 * doctorId is always taken from req.user.id (client value ignored).
 */
router.get("/dashboard", async (req, res) => {
  ...
  if (!startDate || !endDate || !bucket) {
    return res.status(400).json({ error: "startDate, endDate, and bucket are required." });
  }
  ...
  // Enforce a reasonable date range limit (max 366 days)
  const diffDays = Math.round((overallEnd.getTime() - overallStart.getTime()) / 86400000);
  if (diffDays > 366) {
    return res.status(400).json({ error: "Date range must not exceed 366 days." });
  }
  ...
  try {
    ...
    const completedAppointments = await prisma.appointment.findMany({
      ...
      include: {
        patient: { select: { id: true, gender: true, birthDate: true } },
      },
    });

    const buckets = generateBuckets(String(startDate), String(endDate), String(bucket));

    const daysInRange = Math.round((overallEnd.getTime() - overallStart.getTime()) / 86400000);
    ...
    const series = buckets.map((b) => {
      ...
      for (const inv of invoices) {
        const contrib = calcDashboardInvoiceContribution(
          inv,
          b.start,
          b.end,
      ...
      if (String(bucket) === "month") {
        ...
        const daysInBucket = Math.round((b.end.getTime() - b.start.getTime()) / 86400000);
        const weeklyGoal =
    ...
    console.error("GET /api/doctor/dashboard error:", err);
