...
const BOTTOM_NAV = [
  { label: "Цагууд", shortLabel: "Цаг", href: "/doctor/appointments", icon: "calendarDays" as const },
  { label: "Хуваарь", shortLabel: "Хув", href: "/doctor/schedule", icon: "calendarRange" as const },
  { label: "Борлуулалт", shortLabel: "₮", href: "/doctor/sales", icon: "tugrik" as const },
  { label: "Профайл", shortLabel: "Проф", href: "/doctor/profile", icon: "user" as const },
];
...
function BottomIcon({ kind, active }: { kind: (typeof BOTTOM_NAV)[number]["icon"]; active: boolean }) {
  const cls = classNames("h-[22px] w-[22px]", active ? "text-[#131a29]" : "text-gray-400");
      ...
      <header className="fixed top-0 left-0 right-0 h-14 text-white z-[100]" style={{ background: NAVY }}>
        <div className="h-full w-full px-3 flex items-center justify-between sm:max-w-[720px] sm:mx-auto">
          ...
          <Link href="/doctor/appointments" className="min-w-0 flex items-center gap-2 no-underline text-white">
            ...
            <img src="https://mdent.cloud/mdent.svg" alt="mDent" className="h-7 w-7 shrink-0" />
