// frontend/components/patients/PatientsIndexPage.tsx
// Shared patients list page UI used by both admin (/patients) and reception (/reception/patients).
// Props control which parts are shown and how links/routing behave.
          ...
          <form className="p-4" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                />
                <span className="text-xs text-gray-400">
                  Гадаад улсын иргэн бол улсын нэрийг оруулна уу
                </span>
              </div>
...
interface PatientsIndexPageProps {
  /** Show the 4 summary stat cards at the top (admin: true, reception: false) */
  showSummaryCards?: boolean;
  /** CSS class(es) for the outermost container element */
  containerClassName?: string;
  /** Base path for patient profile links, e.g. "/patients" or "/reception/patients" */
  patientProfileBasePath?: string;
  /** When true, always route booking to /reception/appointments */
  isReceptionContext?: boolean;
}
...
export default function PatientsIndexPage({
  ...
  isReceptionContext = false,
}: PatientsIndexPageProps) {
  ...
  }, [debouncedSearch, page, limit, sort, dir, refreshKey, loadPatients]);
  ...
  return (
    <main className={containerClassName}>
      ...
      {/* Summary cards row — only shown in admin context */}
      {showSummaryCards && (
                  ...
                  <tr key={p.id} className="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
                    ...
                    <td className="border-b border-gray-100 py-2 px-3">
                      {(() => {
                        ...
                        const baseUrl = `${patientProfileBasePath}/${encodeURIComponent(p.patientBook.bookNumber)}`;
                        ...
                        return (
                          <div className="flex items-center gap-1">
                            ...
                            {/* Үйлчлүүлэгчийн карт */}
                            <div className="group relative inline-block">
                              <a href={`${baseUrl}?tab=patient_history`} aria-label="Үйлчлүүлэгчийн карт" className={btnCls}>
                                <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                              ...
                              <span className={tooltipCls}>Үйлчлүүлэгчийн карт</span>
                            </div>
                            {/* Цагууд */}
                            <div className="group relative inline-block">
                              <a href={`${baseUrl}?tab=appointments`} aria-label="Цагууд" className={btnCls}>
                                <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            {/* Карт бөглөх */}
                            <div className="group relative inline-block">
                              <a href={`${baseUrl}?tab=visit_card`} aria-label="Карт бөглөх" className={btnCls}>
                                <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <div className="group relative inline-block">
                              ...
                              <span className={tooltipCls}>Цаг захиалах</span>
                            </div>
                          </div>
                        );
                      })()}
