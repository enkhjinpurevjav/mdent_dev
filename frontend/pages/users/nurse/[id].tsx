...
export default function NurseProfilePage() {
  ...
  // selected multiple branches (NurseBranch or generic DoctorBranch join)
  ...
  const resetFormFromNurse = () => {
    if (!nurse) return;
    setForm({
      name: nurse.name || "",
      ovog: nurse.ovog || "",
      email: nurse.email || "",
      branchId: nurse.branchId ? String(nurse.branchId) : "",
      regNo: nurse.regNo || "",
      phone: nurse.phone || "",
    });
  };
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!rRes.ok) {
          setError(rData?.error || "Сувилагчийн мэдээллийг ачааллаж чадсангүй");
          setLoading(false);
          return;
        }
        ...
        setForm({
          ...
          ovog: n.ovog || "",
          email: n.email || "",
          branchId: n.branchId ? String(n.branchId) : "",
          regNo: n.regNo || "",
          phone: n.phone || "",
        });
    ...
    async function loadSchedule() {
      ...
      try {
        ...
        const res = await fetch(
          `/api/users/${id}/nurse-schedule?from=${from}&to=${to}`
        );
  ...
  const handleSaveBranches = async () => {
    ...
    try {
      // reuse generic branches endpoint (DoctorBranch join)
  ...
  const handleDeleteUser = async () => {
    ...
    const ok = window.confirm(
      "Та энэхүү сувилагчийн аккаунтыг устгахдаа итгэлтэй байна уу?"
    );
    if (!ok) return;
  ...
  const handleSaveSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/users/${id}/nurse-schedule`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  const startEditRow = (s: NurseScheduleDay) => {
  ...
  const handleDeleteSchedule = async (scheduleId: number) => {
    ...
    try {
      const res = await fetch(
        `/api/users/${id}/nurse-schedule/${scheduleId}`,
        {
          method: "DELETE",
        }
      );
  ...
  }, [nurse?.branchId, branches]);

  const nurseAssignedBranches: Branch[] =
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-col gap-1 text-[13px]">
              <button
                ...
                className={`text-left px-2.5 py-1.5 rounded-md border-0 cursor-pointer ${activeTab === "profile" ? "bg-blue-50 text-blue-700 font-medium" : "bg-transparent text-gray-500 font-normal"}`}
              >
                Профайл
              </button>
              ...
              <button
                ...
                className={`text-left px-2.5 py-1.5 rounded-md border-0 cursor-pointer ${activeTab === "schedule" ? "bg-blue-50 text-blue-700 font-medium" : "bg-transparent text-gray-500 font-normal"}`}
              >
                Ажлын хуваарь
              </button>
            </div>
            ...
            <>
                ...
                {!isEditingProfile ? (
                  <div className="grid [grid-template-columns:repeat(auto-fit,minmax(220px,1fr))] gap-3 text-[13px]">
                    ...
                    <div>
                      ...
                      <div>{mainBranchName || nurse.branchId || "-"}</div>
                    </div>
                  </div>
                ) : (
                  <form
              ...
              <div className="rounded-xl border border-gray-200 p-4 bg-white">
                ...
                <div className="text-gray-500 text-[13px] mb-2.5">
                  Энэ сувилагч аль салбаруудад ажиллахыг доороос сонгоно уу.
                </div>
